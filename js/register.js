...
//    	console.log($(this).attr('event').toString() + " button clicked");
  //  	console.log($(this).find('.tx').text().trim().toLowerCase());
        if($('#nameDummy').text()==''){
            $('#login_signup_div').fadeIn(500);
            $('#lif').show('fast');
            return;
        }
        if($(this).find('.tx').text().trim().toLowerCase()=="register" && ($(this).attr('status')==null||$(this).attr('status')=='')) {
